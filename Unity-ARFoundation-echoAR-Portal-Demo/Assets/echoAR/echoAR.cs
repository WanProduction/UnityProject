...
public class echoAR : MonoBehaviour
{
    ...
    public void DownloadEntryAssets(Entry entry, string serverURL)
    {
        ...
        if (true)
        {
            ...
            // Get video hologram
            if (entry.getHologram().getType().Equals(Hologram.hologramType.VIDEO_HOLOGRAM))
            {
                // Get video
                VideoHologram videoHologram = (VideoHologram)entry.getHologram();

                GameObject videoSurface; // Primitive that the video will appear on

                // Assign plane to videoSurface
                videoSurface = GameObject.CreatePrimitive(PrimitiveType.Plane);

                // Scale and rotate the plane
                videoSurface.transform.Rotate(new Vector3(90, 180, 0)); // Right way up
                videoSurface.transform.localScale = new Vector3(2, 1, 1); // Generic video dimensions

                // Attach VideoPlayer to videoSurface
                var videoPlayer = videoSurface.AddComponent<VideoPlayer>();
                videoPlayer.playOnAwake = false;

                // Attach a CustomBehaviour Component
                videoSurface.AddComponent<CustomBehaviour>().entry = entry;

                // Set gameobject name to video name
                videoSurface.name = videoHologram.getFilename();

                // Set video URL
                videoPlayer.url = serverURL + "&file=" + videoHologram.getStorageID();

                // Play
                Debug.Log("Preparing to play video...");
                videoPlayer.Play();
            }
        }
    }
    ...
    IEnumerator WebsocketClient()
    {
        ...
        WClient.On(WClient.EventType.DELETE_ENTRY.ToString(), (string message) => {
            ...
            // Check if item deleted is the portal Video
            // Get skydome
            GameObject skyDome = GameObject.FindGameObjectsWithTag("skyDome")[0];
            Entry portalVidEntry = skyDome.GetComponent<PortalVideoManager>().entry;
            // Check if deleted item entryID matches portalVideo entry ID
            if (portalVidEntry != null && entryID.Equals(portalVidEntry.getId())) 
            {
                // Remove VideoPlayer from skyDome
                Destroy(skyDome.GetComponent<PortalVideoManager>().videoPlayer);
                skyDome.GetComponent<PortalVideoManager>().videoURL = null;
            }
